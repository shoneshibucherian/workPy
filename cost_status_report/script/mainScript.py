...
cost_date_formate=config["cost"]["date"]
...
status_report=os.path.join(input_dir,"status.xlsx")
cost_report=os.path.join(input_dir,"cost.xlsx")

creatDic(status_report)
add_est_cost(cost_report)
# pprint.pprint(month_project)
create_formatted_excel(dict, 0 , "day_wise_Status_Report" ,os.path.join(output_dir, "status_project_report.xlsx"), )
create_formatted_excel(month_project,1,"month_wise_Status_Report",  os.path.join(output_dir, "status_project_report.xlsx"), )
time.sleep(5.0)