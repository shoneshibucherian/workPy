...
def create_summary_report(total_summary_data, newF=1):
    # Create a new workbook and select the active worksheet
    output_dir = os.path.join(os.getcwd(), 'output')
    if newF==1:
        
        
        wb=openpyxl.load_workbook(os.path.join(output_dir, "Entry_and_Exit_of_Bus.xlsx"))
    ...
    ws['Z1'] = 'VARIANCE BETWEEN\nPROJECT REPORT AND WORKSHOP REPORT'
    ws.merge_cells('Z1:AD1')
    ...
    Tafter_10=0
    Tnot_report=0
    Trelease_on_same_day=0
    Tpending_on_same_day=0
    ...
    for idex, datee in enumerate(total_summary_data, start=1):
        ...
        diff=NMC-REPORTED
        print(total_summary_data[datee])
        after_10=total_summary_data[datee]["REPORTED AFTER 10 AM ON THE SAME DAY"]
        release_on_same_day=total_summary_data[datee]["RELEASED_SAME_DAY"]
        pending_on_same_day=total_summary_data[datee]["PENDING_SAME_DAY"]

        Tpending_on_same_day+=pending_on_same_day
        Trelease_on_same_day+=release_on_same_day
        ...
        not_report=diff-after_10
        Tnot_report+=not_report
        ...
        row=[idex, datee, NMC, REPORTED, RELEASED, PENDING,
              ...
              " ",
              diff, after_10, release_on_same_day,pending_on_same_day,not_report]
    ...
    total_data = ["TOTAL",TNMC, TREPORTED, TRELEASED, TPENDING,
                  TPM_IN, TPM_OUT, TPM_PENDING,
                  ...
                  0, 0, 0,  # Placeholder for W/S OR WINDOW GLASS,
                  " ",
                  Tdiff, Tafter_10,Trelease_on_same_day, Tpending_on_same_day,Tnot_report]
    for col_idx, value in enumerate(total_data, start=2):
        ...
        ws.cell(row=total_row, column=col_idx).alignment = center_alignment
        ws.cell(row=total_row, column=col_idx).fill = sub_header_fill

    # Apply formatting to all header cells (rows 1-3) including merged cell ranges
    for row in range(1, 4):
        for col in range(1, 31):  # Adjust range as needed
    ...
    merged_ranges = [
        ...
        ('Z2:Z3', 'medium'), # Total Differences
        ('AA2:AA3', 'medium'), # REPORTED AFTER 10 AM
        ('AB2:AB3', 'medium'), # RELEASED ON THE SAME DAY
        ('AC2:AC3', 'medium'), # PENDING
        ('AD2:AD3', 'medium'), # NOT REPORTED ON THE SAME DAY
    ]
    ...
    ws.cell(row=summary_row, column=4).value = int(TREPORTED) if TREPORTED else 0
    ws.cell(row=summary_row, column=6).value = 'NO: OF BUSES\nRELEASED'
    ws.cell(row=summary_row, column=8).value = int(TRELEASED) if TRELEASED else 0
    ws.cell(row=summary_row, column=10).value = 'OUTPUT\nEFFICIENCY'
    
    # Calculate efficiency as a simple percentage
    if TREPORTED and TREPORTED != 0:
        efficiency = round((TRELEASED / TREPORTED) * 100, 1)
        ws.cell(row=summary_row, column=12).value = f"{efficiency}%"
    else:
        ws.cell(row=summary_row, column=12).value = "0%"

    ws.cell(row=summary_row, column=14).value = 'NO: OF BUSES NOT\nREPORTED ON THE SAME DAY'
    ws.cell(row=summary_row, column=17).value = Tnot_report
    ...
    summary_merged_ranges = [
        ...
        ('N{}:Q{}'.format(summary_row, summary_row))
    ]
    ...
    for col in range(26, 31):  # Columns Y through AC
        column_widths[col] = 8
    ...
    # Save the workbook with a simple filename to avoid conflicts
    filename = os.path.join(output_dir, "Entry_and_Exit_of_Bus.xlsx")
    wb.save(filename)
    print(f"✅ Status Project Report table created successfully!")
